...
export default class World extends PIXI.Container {
    ...
    createSmallCloud(key) {
        ...
        cloud.position.set(Math.random() * this.boundary.width, Math.random() * (this.horizon - cloud.height))
        smallClouds.addChild(cloud)

        gsap.timeline({ repeat: -1 })
            .fromTo(cloud, (this.boundary.width - cloud.position.x) * cloud.vx, { x: cloud.position.x }, { x: this.boundary.width, ease: 'none' }, 0)
            .fromTo(cloud, (cloud.position.x + cloud.width) * cloud.vx, { x: -cloud.width }, { x: cloud.position.x, ease: 'none' }, (this.boundary.width - cloud.position.x) * cloud.vx)
    }
    ...
    moveCamera() {
        ...
        if ((this.y < 0 && boat.netUp && net.getBounds().top < innerHeight * 0.25) ||
            (this.y > -this.height + this.width / innerWidth * innerHeight &&
                boat.netDown && net.getBounds().bottom > innerHeight * 0.75)) {
            this.y -= net.vy
        }
    }
